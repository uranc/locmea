...
if not os.path.isfile('L5bPCmodelsEH/morphologies/cell1.asc'):
    # get the model files:
    u = urlopen(
        'http://senselab.med.yale.edu/ModelDB/eavBinDown.asp?o=139653&a=23&mime=application/zip')
...
X, Y, Z = np.mgrid[-2:2:1, 0:4:1, -8:8:1] * 20
# define parameters for extracellular recording electrode, using optional
# method
electrodeParameters = {
    ...
    'r': 7.5,              # contact site radius
    'n': 50,               # datapoints for averaging
}
...
#cell.set_rotation(x=4.99, y=-4.33, z=0)
cell.set_pos(ypos=40)
...
cell.simulate(electrode=electrode, rec_imem=True, to_file=True,
        file_name="name.h5")
...
def plotstuff(cell, electrode):
    ...
    ax1.plot(electrode.x[ind], electrode.z[ind], '*',
             markersize=5, markeredgecolor='none', markerfacecolor='k')
...
plt.show()
# save file as h5py
electrode_out = np.concatenate((np.reshape(electrode.x, (electrode.x.size, 1)),
                         np.reshape(electrode.y, (electrode.y.size, 1)),
                         np.reshape(electrode.z, (electrode.z.size, 1)),
                         electrode.LFP),
                         axis=1)
cell_out = np.concatenate((np.reshape(cell.xstart, (cell.xstart.size, 1)),
                          np.reshape(cell.ystart, (cell.ystart.size, 1)),
                          np.reshape(cell.zstart, (cell.zstart.size, 1)),
                          np.reshape(cell.xmid, (cell.xmid.size, 1)),
                         np.reshape(cell.ymid, (cell.ymid.size, 1)),
                         np.reshape(cell.zmid, (cell.zmid.size, 1)),
                         np.reshape(cell.xend, (cell.xend.size, 1)),
                         np.reshape(cell.yend, (cell.xend.size, 1)),
                         np.reshape(cell.zend, (cell.xend.size, 1)),
                         cell.imem), axis=1)
with h5py.File("data_sim_low.hdf5", "w") as f:
    f['srate'] = 1000. / cellParameters['timeres_python']
    f.create_dataset("cell", data=cell_out)
    f.create_dataset("electrode", data=electrode_out)
